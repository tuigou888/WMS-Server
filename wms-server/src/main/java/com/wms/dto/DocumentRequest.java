...
public record DocumentRequest(@NotBlank @Pattern(regexp="IN|OUT",message="类型必须为 IN 或 OUT") String type, Long partnerId, @NotNull(message="仓库不能为空") Long warehouseId, LocalDate businessDate, String remark, @NotEmpty(message="至少需要一条明细") List<@Valid DocumentLineRequest> lines) {
