...
public class InventoryService {
 ...
 @Transactional public Map<String,Object> stockOut(StockOutRequest request,String referenceNo,String type){
  ...
  if(inventory.getQuantity().compareTo(request.quantity())<0) throw new BusinessException("库存不足：可用 "+inventory.getQuantity().stripTrailingZeros().toPlainString()); BigDecimal unit=inventory.getAvgCost();BigDecimal cost=InventoryCostCalculator.amount(request.quantity(),unit);BigDecimal sales=InventoryCostCalculator.amount(request.quantity(),request.salePrice());BigDecimal profit=sales.subtract(cost).setScale(2,RoundingMode.HALF_UP);BigDecimal quantity=inventory.getQuantity().subtract(request.quantity());BigDecimal total=inventory.getTotalAmount().subtract(cost).max(BigDecimal.ZERO).setScale(2,RoundingMode.HALF_UP);BigDecimal avg=quantity.signum()==0?BigDecimal.ZERO.setScale(4):unit;
 ...
 private Item item(String code){return items.findByCode(code).orElseThrow(()->new BusinessException("未找到物品编码："+code));} private Warehouse warehouse(Long id){return warehouses.findById(id).orElseThrow(()->new BusinessException("仓库不存在"));}
 private Location location(Warehouse warehouse,String code,boolean create){return locations.findByWarehouseIdAndCode(warehouse.getId(),code).orElseGet(()->{if(!create)throw new BusinessException("库位不存在："+code);return locations.save(new Location(warehouse,code));});}
