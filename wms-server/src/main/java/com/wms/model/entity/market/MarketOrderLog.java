...
public class MarketOrderLog extends AuditableEntity {
    ...
    @Column(nullable = false, length = 30) private MarketOrderAction action;          // CREATE / PAY / AUDIT / SHIP / COMPLETE / CANCEL / REFUND
    @Column(length = 60) private String operator;                          // 操作人用户名（系统操作为 system）
    @Column(length = 500) private String remark;

    public MarketOrderLog() {}
    public MarketOrderLog(Long orderId, MarketOrderAction action, String operator, String remark) {
    ...
    public MarketOrderAction getAction() { return action; } public void setAction(MarketOrderAction action) { this.action = action; }
