...
import com.wms.common.*; import com.wms.dto.LoginRequest; import com.wms.dto.UserRequest; import com.wms.dto.WxLoginRequest; import com.wms.dto.WxBindRequest; import com.wms.dto.WxRegistrationRequest; import com.wms.model.entity.UserAccount; import com.wms.repository.UserAccountRepository; import com.wms.security.*; import com.wms.service.LoginRateLimiter; import com.wms.service.WechatBindTicketService; import com.wms.service.WechatService; import com.wms.service.WarehouseAccessService; import jakarta.servlet.http.HttpServletRequest; import jakarta.validation.Valid; import org.slf4j.Logger; import org.slf4j.LoggerFactory; import org.springframework.dao.DataIntegrityViolationException; import org.springframework.security.access.prepost.PreAuthorize; import org.springframework.security.core.context.SecurityContextHolder; import org.springframework.security.crypto.password.PasswordEncoder; import org.springframework.transaction.annotation.Transactional; import org.springframework.web.bind.annotation.*; import java.util.*;
@RestController @RequestMapping("/auth") public class AuthController {
  private static final Logger log = LoggerFactory.getLogger(AuthController.class);
  private final UserAccountRepository users; private final PasswordEncoder encoder; private final TokenService tokens; private final WechatService wechat; private final LoginRateLimiter rateLimiter; private final WechatBindTicketService bindTickets; private final WarehouseAccessService warehouseAccess;
  public AuthController(UserAccountRepository users,PasswordEncoder encoder,TokenService tokens,WechatService wechat,LoginRateLimiter rateLimiter,WechatBindTicketService bindTickets,WarehouseAccessService warehouseAccess){this.users=users;this.encoder=encoder;this.tokens=tokens;this.wechat=wechat;this.rateLimiter=rateLimiter;this.bindTickets=bindTickets;this.warehouseAccess=warehouseAccess;}
 @PostMapping("/login") public ApiResponse<Map<String,Object>> login(@Valid @RequestBody LoginRequest request,HttpServletRequest http){String username=normalizeUsername(request.username());String key=rateKey(http,username);if(rateLimiter.isBlocked(key))throw new RateLimitedException("登录失败次数过多，请 "+LoginRateLimiter.WINDOW.toMinutes()+" 分钟后再试");UserAccount user=users.findByUsername(username).orElse(null);if(user==null||!Boolean.TRUE.equals(user.getEnabled())||!encoder.matches(request.password(),user.getPassword())){rateLimiter.recordFailure(key);throw new BusinessException("用户名或密码错误");}rateLimiter.reset(key);String token=tokens.issue(user);log.info("登录成功: username={}, role={}", user.getUsername(), user.getRole());return ApiResponse.ok("登录成功",view(user,token));}
  ...
  @GetMapping("/users") @PreAuthorize("hasAuthority('user:manage')") public ApiResponse<List<Map<String,Object>>> users(){SecurityUtils.require(Permissions.USER_MANAGE);return ApiResponse.ok(users.findAll().stream().map(this::userView).toList());}
 @PostMapping("/users") @Transactional @PreAuthorize("hasAuthority('user:manage')") public ApiResponse<Map<String,Object>> createUser(@Valid @RequestBody UserRequest request){SecurityUtils.require(Permissions.USER_MANAGE);if(users.findByUsername(request.username().trim()).isPresent())throw new BusinessException("用户名已存在");if(request.password()==null||request.password().length()<6)throw new BusinessException("密码至少 6 位");UserAccount u=new UserAccount();u.setUsername(request.username().trim());u.setPassword(encoder.encode(request.password()));u.setDisplayName(request.displayName());u.setRole(validRole(request.role()));u.setEnabled(request.enabled()==null||request.enabled());UserAccount saved=users.save(u);warehouseAccess.replaceAssignments(saved,request.warehouseIds());return ApiResponse.ok("用户创建成功",userView(saved));}
 @PutMapping("/users/{id}") @Transactional @PreAuthorize("hasAuthority('user:manage')") public ApiResponse<Map<String,Object>> updateUser(@PathVariable Long id,@Valid @RequestBody UserRequest request){SecurityUtils.require(Permissions.USER_MANAGE);UserAccount u=users.findById(id).orElseThrow(()->new BusinessException("用户不存在"));u.setDisplayName(request.displayName());u.setRole(validRole(request.role()));u.setEnabled(request.enabled()==null||request.enabled());if(request.password()!=null&&!request.password().isBlank()){if(request.password().length()<6)throw new BusinessException("密码至少 6 位");u.setPassword(encoder.encode(request.password()));}UserAccount saved=users.save(u);warehouseAccess.replaceAssignments(saved,request.warehouseIds());tokens.revokeByUsername(saved.getUsername());return ApiResponse.ok("用户更新成功",userView(saved));}
 @PostMapping("/logout") public ApiResponse<Void> logout(@RequestHeader(value="Authorization",required=false) String auth){if(auth!=null&&auth.startsWith("Bearer ")){tokens.revoke(auth.substring(7));log.info("用户退出登录");}return ApiResponse.ok("已退出登录",null);}
  ...
  private Map<String,Object> userView(UserAccount u){Map<String,Object> view=new LinkedHashMap<>();view.put("id",u.getId());view.put("username",u.getUsername());view.put("displayName",u.getDisplayName()==null?u.getUsername():u.getDisplayName());view.put("role",u.getRole());view.put("enabled",u.getEnabled());view.put("warehouseIds",warehouseAccess.assignedWarehouseIds(u.getId()));return view;}
