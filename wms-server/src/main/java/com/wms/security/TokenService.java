...
public class TokenService {
    public record Principal(String username,String role,String displayName) {}
    private record Session(Principal principal, Instant expiresAt) {}
    private final Map<String,Session> sessions = new ConcurrentHashMap<>();
    public String issue(UserAccount user){ cleanup(); String token=UUID.randomUUID().toString().replace("-","")+UUID.randomUUID().toString().replace("-",""); sessions.put(token,new Session(new Principal(user.getUsername(),user.getRole(),user.getDisplayName()),Instant.now().plus(Duration.ofHours(12)))); return token; }
