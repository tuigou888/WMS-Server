...
const load = (target = page.value) => { loading.value = true; return api.documents({ page: target, pageSize: 20 }).then((x) => { data.value = x.records; page.value = x.page; total.value = x.total }).catch((e) => message.error(e.message)).finally(() => { loading.value = false }) }
