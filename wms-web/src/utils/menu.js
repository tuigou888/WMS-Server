...
export function buildMenu(user) {
  return [
    ...
    ...((hasPerm(user, 'inventory:scan') || hasPerm(user, 'document:read') || hasPerm(user, 'adjustment:read') || hasPerm(user, 'transfer:read') || hasPerm(user, 'stocktake:read') || hasPerm(user, 'inventory:read')) ? [{ type: 'group', label: '仓储业务', children: [
      ...(hasPerm(user, 'inventory:scan') ? [{ key: '/stock-in', icon: h(InboxOutlined), label: '扫码入库' }] : []),
      ...(hasPerm(user, 'inventory:scan') ? [{ key: '/stock-out', icon: h(LogoutOutlined), label: '扫码出库' }] : []),
    ...
    ...(hasPerm(user, 'report:view') ? [{ type: 'group', label: '数据中心', children: [
      ...
      ...(hasPerm(user, 'qrcode:read') ? [{ key: '/tools', icon: h(QrcodeOutlined), label: '二维码与电子表格' }] : []),
    ] }] : []),
    ...
    ...((hasPerm(user, 'product:read') || hasPerm(user, 'order:read') || hasPerm(user, 'customer:read') || hasPerm(user, 'report:view')) ? [{ type: 'group', label: '商城管理', children: [
      ...(hasPerm(user, 'report:view') ? [{ key: '/market/dashboard', icon: h(DashboardOutlined), label: '商城概览' }] : []),
